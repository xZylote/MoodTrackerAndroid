...
function changeDate() {
    let start = document.getElementById("visStart").value;
    let end = document.getElementById("visEnd").value;
    if (start != "" && end != "") {
        ...
        window.location = "https://m.hay.li/hidden.html?q=" + uid + "&s=" + start.replace(/\-/g, "/") + "&e=" + end.replace(/\-/g, "/");
    }
}
...
firebase.database().ref('/Users/' + uid + '/moods').once('value').then(function(snapshot) {
    ...
    d3.json(url).then(function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            document.getElementById("rec" + i).style.top = 105 + max + "px";
            document.getElementsByClassName("rectangle")[i].setAttribute("height", height - max + 300 + "px");
            document.getElementsByClassName("rectangle")[i].parentElement.setAttribute("height", height - max + 300 + "px");
            document.getElementById("rec" + i).style.left = 107.5 + x(data[i].time) + "px";
            ...
            document.getElementById("div" + i).style.top = "1100px";
            if (data[i].time) {
                document.getElementById("div" + i).style.left = 92.5 + x(data[i].time) + "px";
            }
            ...
            if (data[i].time) {
                document.getElementById("comp" + i).style.left = 80 + x(data[i].time) + "px";
            }

            if (data[i].companions != "") {
                var max = Math.min(y(data[i].mood), y(data[i].stress))
                document.getElementById("comp" + i).style.top = max - 895 + "px";
